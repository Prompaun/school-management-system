...
const handlePreviousSchoolEducationalRecordsFileChange = (event) => {
    ...
    useEffect(() => {
      ...
      const checkFather_Email = async (email) => {
        try {
            const response = await axios.get(process.env.api + `check-email?email=${email}`);
  ...
  const handleSubmit = async (Student_picture_file, CopyofStudentIDCardFile, PreviousSchoolEducationalRecordsFile, studentNID, nameTitle, FirstName, LastName, Student_DateOfBirth, Transcript_type, HouseNumber, Moo, Soi, Road, Province, District, SubDistrict, HouseReg_file) => {
    ...
    // if (confirmSubmit) {
      ...
      try {
          ...
          await axios.post(process.env.api + 'upload', formData, {
            headers: {
  ...
  async function checkEnrollment(Student_NID, Enroll_Date, Enroll_Year, Enroll_Course) {
    try {
        const check_enrollment_response = await axios.get(process.env.api + `check-student-enrollment?Student_NID=${Student_NID}&Enroll_Year=${Enroll_Year}&Enroll_Course=${Enroll_Course}`);
        ...
        if (data.length > 0) {
            ...
            try {
                ...
                const save_enrollment_response = await axios.post(process.env.api + 'enrollment', formData);
  ...
  const addParentEmails = async (Student_NID, first_ParentEmail, second_ParentEmail, third_ParentEmail) => {
    try {
        ...
        const response = await axios.post(process.env.api + 'add-parent-emails', requestData);
  ...
  const addParentInformation = async (Avatar, Email, FirstName, LastName, DateOfBirth, Nationality, Office, Occupation, Role, Tel) => {
    try {
      const parentData = [{
        ...
        const response = await axios.post(process.env.api + 'Parent_information', parentData);
        console.log(response.data.message);
        return response.data.message;
    } catch (error) {
        if (error.response) {
